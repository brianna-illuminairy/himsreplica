...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      // Sanity bound: if the persisted offset is wildly off-screen (e.g.
      // content scrolled tens of thousands of px away), discard it so we
      // don't restore the user into an empty void. 20000px gives plenty
      // of pan headroom for any reasonable canvas while still catching
      // the runaway case.
      const sane = s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)
        && Math.abs(s.x) < 20000 && Math.abs(s.y) < 20000;
      if (sane) {
